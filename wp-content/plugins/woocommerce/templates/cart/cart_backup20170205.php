...
if ( $count > 0 ){
    ...
    foreach ( $product_categories as $product_category ) {
        ...
        if($product_category->slug == '1course') {
            echo "<select id='productname' onchange='getproductdetails()'><option value='' selected>Course Type</option>";
...
//            echo "</div>";
        ...
        if($product_category->slug == '2accomdation') {
            echo "<select id='accomdation' onchange='allowShowOthers()' disabled><option value='' selected>No Accomdation Thanks</option>";
            while ( $products->have_posts() ) {
                $products->the_post();
                ?>
                    <option value="<?php the_id(); ?>">
...
//			echo '<h4 ><div class="extra-second-section">' . $product_category->name . '</div></h4>';
             echo "<select name='visa_require' id='visa_require' onchange='getvisaitems()'>";
            while ( $products->have_posts() ) {
                $products->the_post();
                ?>
                    <option value="<?php the_id(); ?>">
...
//                jQuery("#coursestartdate").html(result);
    ...
    function getcourselevel(s){
        ...
        if(!course_level){
        var product_id = jQuery( "#productname option:selected" ).val();
        jQuery.ajax({
            type: 'post',
...
//                          console.log("Inside false Current Date"+dmy);
    ...
    function setWeeks(){
        var id = jQuery( "#productname option:selected" ).val();
        jQuery.ajax({
            type: 'post',
...
//            autoclose:true
    ...
    jQuery( document ).ready( function(jQuery) {
       ...
       function courseUpdate() {
            var course_level = jQuery( "#course_level :selected" ).text();
            if(course_level){
            var p_id = jQuery( "#productname option:selected" ).val();
