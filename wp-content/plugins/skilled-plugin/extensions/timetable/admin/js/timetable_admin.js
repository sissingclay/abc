jQuery(document).ready(function($){
	...
	if($("#timetable_settings").length)
		$("#timetable_settings")[0].reset();
	$("#event, #event_category, #hour_category, #weekday, #measure, #filter_style, #filter_kind, #timetable_settings [name='time_format'], #timetable_settings [name='time_format_custom'], #hide_hours_column, #hide_all_events_view, #show_end_hour, #hide_empty, #disable_event_url, #text_align, #responsive, #direction, #event_layout, #timetable_font, #timetable_font_subset").change(function(event, param){
	...
	$(document).on("change", ".vc_panel .vc_shortcode-param select.font", function(event) {
		var $this = $(this);
		var font = $this.val();
		if(font==="")
			return;
		$.ajax({
			...
			success: function(data){
				...
				var $subset = $(".vc_panel .vc_shortcode-param select.font_subset");
	...
	$(document).on("change", ".vc_panel .vc_shortcode-param select.select_time", function(event) {
		var $this = $(this);
		var $time_format = $(".vc_panel .vc_shortcode-param input.time_format");
		var value = $this.val();
...
function timetable_font_subset_init() {
	var $ = jQuery;
	var $google_font = $(".vc_panel .vc_shortcode-param select.font");
