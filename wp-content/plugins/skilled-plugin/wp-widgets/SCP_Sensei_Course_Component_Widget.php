 ...
 * @since 1.1.0
 */
class SCP_Sensei_Course_Component_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$this->woo_widget_idbase = 'skiled_course_component';
		$this->woo_widget_title = __( 'Skilled - Course Component', 'woothemes-sensei' );
	...
	public function update ( $new_instance, $old_instance ) {
		...
		$instance['title_num_words'] = (int) $new_instance['title_num_words'];
		...
		$instance['show_title_only'] = $new_instance['show_title_only'];
...
?>
		...
		<!-- Widget Title Number of Words: Text Input -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title_num_words' ) ); ?>"><?php _e( 'Number of Words in the Title (optional):', 'woothemes-sensei' ); ?></label>
			<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'title_num_words' ) ); ?>"  value="<?php echo esc_attr( $instance['title_num_words'] ); ?>" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title_num_words' ) ); ?>" />
		</p>
		...
		<!-- Widget Show Title Only: Checkbox Input -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_title_only' ) ); ?>"><?php _e( 'Show Title Only (optional):', 'woothemes-sensei' ); ?></label>
			<input type="checkbox" name="<?php echo esc_attr( $this->get_field_name( 'show_title_only' ) ); ?>"  <?php checked( $instance['show_title_only'], 'on' ); ?>class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'show_title_only' ) ); ?>" />
		</p>
		...
		if ( count( $posts_array ) > 0 ) { ?>
		    ...
		    ?>
		    	<li class="fix">
		    		...
		    		<a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>" title="<?php echo esc_attr( $post_title ); ?>">
						<?php if ($instance['title_num_words']): ?>
							<?php echo wp_trim_words($post_title, $instance['title_num_words'], ''); ?></a>
						<?php else: ?>
							<?php echo $post_title; ?>
						<?php endif; ?>

					<?php if ($instance['show_title_only'] != 'on'): ?>
			    		<br />
...
register_widget('SCP_Sensei_Course_Component_Widget');
