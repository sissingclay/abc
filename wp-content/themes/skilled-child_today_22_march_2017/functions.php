...
//                     }
                        ...
                        if(isset($_SESSION['under_18']) && !empty($_SESSION['under_18']) && $_SESSION['under_18'] == 'yes' && $getAccUnder18Fees != 'yes'){
                            ...
                            foreach ($wc_extra_cost_based_on_product_tag as $keyTagCostAcc) {  

                                $prod_tag_amount = $keyTagCostAcc['extra_cost_based_product_tag_amount']*$_POST['back_accomodation_week'];                               
                                
                                echo '<tr><td><ul class="check-list"><li><div class="acc-supply">'.$keyTagCostAcc['extra_cost_based_product_tag_title'].'&nbsp;&nbsp;</div></li></ul></td><td class="priceright">'.get_woocommerce_currency_symbol().''.number_format($prod_tag_amount, 2).'</td></tr>'; 
                                $arr_extrasum += $prod_tag_amount;

                                $woocommerce->cart->add_fee($keyTagCostAcc['extra_cost_based_product_tag_title'], $prod_tag_amount, false, '');                                    
                                $getAccUnder18Fees == 'yes';
    ...
    $i=0;
    $price='';
    foreach($items as $item => $values) { //echo '<pre>'; print_r($values);
        $term_list = wp_get_post_terms($values['product_id'],'product_cat',array('fields'=>'ids'));
        $cat_id = (int)$term_list[0];
        $cat_name = get_cat_name ($cat_id, 'product_cat');
        $variation_id = $values['variation_id'];
...
function getaccweek(){
    ...
    //    ini_set("display_errors","1");
         ...
         for($i=1;$i<=$week;$i++){
            ...
            echo "<input type='text' name='acc_startdate' id='acc_startdate' placeholder='Starting Date' value=".$newdate." readonly>";
            echo "</div>";
            
            echo "<div id='accenddate'>";
            echo '<label>Finishing Date</label>';
            echo "<input type='text' name='acc_enddate' id='acc_enddate' placeholder='Finishing Date' value=".$nxtdate." readonly>";
            echo "</div>";
