...
include 'accommodation.functions.php';
include 'course.functions.php';
...
function get_variation_id() {
    ...
    if ($data->courseweek && $data->product_id) {
        ...
        WHERE wp_posts.post_parent='".$data->product_id."' AND wp_posts.ID = wp_postmeta.post_id  AND wp_postmeta.meta_value='".$data->courseweek."'";
        $result = $wpdb->get_results($query);
        wp_send_json( array('variation_id' => intval($result[0]->ID)) );
        die;
    }
}
...
function show_cart_data(){
    ...
    $_SESSION['course']['back_getProductOption'] = $data->back_getProductOption;
    $_SESSION['course']['back_productid'] = $data->back_productid; 
    $_SESSION['course']['back_productidpop'] = $data->back_productid;
    $_SESSION['course']['back_coursestartdate'] = $data->back_coursestartdate;
    $_SESSION['course']['back_courseweek'] = $data->back_courseweek;
    $_SESSION['course']['back_course_level'] = $data->back_course_level;
    

    global $woocommerce;
        $items = $woocommerce->cart->get_cart();
        $arr = urldecode(str_replace("\\","",$data->extracharges));
        $extracharges = json_decode($arr);
        
        foreach($items as $item => $values) {
            $info['term_list'] = wp_get_post_terms($values['product_id'],'product_cat',array('fields'=>'ids'));
            $info['cat_id'] = (int)$term_list[0];
            $info['cat_name'] = get_cat_name ($cat_id, 'product_cat');
            $info['variation_id'] = $values['variation_id'];
            $info['variation'] = wc_get_product($variation_id);
            $info['variation_title_pre'] = $variation->get_formatted_name();
            $info['vt'] = explode("&ndash;",$variation_title_pre);

           $proc_level =$data->course_level;
           $meta1 = get_post_meta($variation_id, 'attribute_pa_week', true);
           $term1 = get_term_by('slug', $meta1, 'pa_week');
           $proc_week =$term1->name;
           $proc_startdate =$data->start_date;

           $info['$product_name'] = $vt[1];
           $info['$proc_price'] = $vt[3];

            if($data->back_productid == "19531") {
            ...
            $info['$valuenew'] = $valuenew;
            $info['$proc_level'] = ucwords($proc_level);
            $info['$proc_week'] = ucwords($proc_week);
            $info['$proc_startdate'] = $proc_startdate;
            $info['$proc_price'] = $proc_price;
            ...
            foreach ($wc_extra_cost_based_on_product as $key => $valProdCost) {
                if($valProdCost['extra_cost_based_product_name'] == $values['product_id'] && $cat_name== 'Courses') {
                    $info['based_product_cost_title'] = array(
                        '$extra_cost_based_product_cost_title' => $keyCatCost['extra_cost_based_product_cost_title'],
                        '$currency_symbol' => get_woocommerce_currency_symbol(),
                        '$extra_cost_based_product_amount' => number_format($keyCatCost['extra_cost_based_product_amount'], 2)
                    );
            ...
            foreach ($wc_extra_cost_based_on_category as $keyCatCost) { 
                if($keyCatCost['extra_cost_based_product_category_name'] == $cat_id && $cat_name== 'Courses') {
                    $info['based_product_category_title'] = array(
                        '$extra_cost_based_product_category_title' => $keyCatCost['extra_cost_based_product_category_title'],
                        '$currency_symbol' => get_woocommerce_currency_symbol(),
                        '$extra_cost_based_product_category_amount' => number_format($keyCatCost['extra_cost_based_product_category_amount'], 2)
                    );
            ...
            $info['$amount'] = number_format($amount2, 2);
