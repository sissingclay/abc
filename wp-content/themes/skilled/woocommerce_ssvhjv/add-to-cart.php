...
if ( WooThemes_Sensei_Utils::sensei_customer_bought_product( $current_user->user_email, $current_user->ID, $wc_post_id ) ) { ?>
    <div class="sensei-message tick"><?php _e( 'You are currently taking this course.', 'skilled' ); ?></div>
<?php } else {
                ...
                <?php if (! sensei_check_if_product_is_in_cart( $wc_post_id ) ) { ?>
                    <form action="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="cart" method="post" enctype="multipart/form-data">
                        ...
                        <?php if ( isset( $product->variation_id ) && 0 < intval( $product->variation_id ) ) { ?>
                            ...
                            <?php } ?>
                        <?php } ?>
                        <button type="submit" class="single_add_to_cart_button button alt"><?php echo apply_filters('single_add_to_cart_text', esc_html__('Purchase this Course', 'skilled'), $product->product_type); ?></button>
                    </form>
...
if ( !is_user_logged_in() ) {
    ...
    $login_link =  '<a href="' . esc_url( get_permalink( $my_courses_page_id ) ) . '">' . esc_html__( 'log in', 'skilled' ) . '</a>'; ?>
    <p class="add-to-cart-login">
        <?php echo sprintf( esc_html__( 'Or %1$s to access your purchased courses', 'skilled' ), $login_link ); ?>
    </p>
<?php } ?>
