...
class Wheels_Theme_Options_Importer implements Wheels_Importer_Interface {
	...
	protected $theme_option_name = SKILLED_THEME_OPTION_NAME;
	protected $filename;
	...
	public function import() {
		...
		if ( ! file_exists( $this->get_filename() ) ) {
			wp_die( __( 'Theme options Import file could not be found. Please try again.', 'skilled' ), '', array( 'back_link' => true ) );
		}
		...
		if ($data) {
			...
			if ( empty( $data ) || ! is_array( $data ) ) {
				wp_die( __( 'Theme options import data could not be read. Please try a different file.', 'skilled' ), '', array( 'back_link' => true ) );
			}
			...
			if ( ! $this->get_theme_option_name() ) {
				wp_die( __( 'Theme options name not defined. Please define it and try again.', 'skilled' ), '', array( 'back_link' => true ) );
			}
